...
void UCombatComponent::PickupFlag(ATeamFlag* Flag)
{
	...
	const USkeletalMeshSocket* BackpackSocket = Character->GetMesh()->GetSocketByName(FName("FlagBackpackSocket"));
	if (BackpackSocket)
	{
	...
	Flag->SetFlagState(EFlagState::EFS_Equipped);
}
...
bool UCombatComponent::CanFire()
{
	...
	if (!EquippedWeapon->IsEmpty() && bCanFire && (CombatState == ECombatState::ECS_Reloading || bLocallyReloading) && EquippedWeapon->GetWeaponType() == EWeaponType::EWT_Shotgun)
	{
		return true;
	}
...
void UCombatComponent::InitialiazeCarriedAmmo()
{
...
void UCombatComponent::SetHUDCrosshairs(float DeltaTime)
{
	...
	if (Controller)
	{
		...
		if (HUD)
		{
			if (EquippedWeapon)
			{
				HUDPackage.CrosshairsCenter = EquippedWeapon->CrossharisCenter;
				HUDPackage.CrosshairsLeft = EquippedWeapon->CrossharisLeft;
				HUDPackage.CrosshairsRight = EquippedWeapon->CrossharisRight;
				HUDPackage.CrosshairsTop = EquippedWeapon->CrossharisTop;
				HUDPackage.CrosshairsBottom = EquippedWeapon->CrossharisBottom;
			}
			else
			{
...
void UCombatComponent::FinishReloading(bool bMontageInterrupted)
{
...
void UCombatComponent::FinishSwap(bool bMontageInterrupted)
{
	if (Character && Character->HasAuthority())
	{
		CombatState = ECombatState::ECS_Unoccupied;
...
void UCombatComponent::FinishSwapAttachWeapons(bool bMontageInterrupted)
{
...
void UCombatComponent::OnRep_SecondaryWeapon(AWeapon* LastSecondaryWeapon)
{
	if (SecondaryWeapon && Character)
	{
		...
		if (LastSecondaryWeapon == nullptr)
		{
