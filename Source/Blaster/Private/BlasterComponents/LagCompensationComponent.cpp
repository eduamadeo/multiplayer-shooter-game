...
FShotgunServerSideRewindResult ULagCompensationComponent::ShotgunConfirmHit(TArray<FFramePackage>& FramePackages, const FVector_NetQuantize TraceStart, const TArray<FVector_NetQuantize>& HitLocations)
{
	for (auto& FramePackage : FramePackages)
	{
		if (FramePackage.Character == nullptr)
		{
	...
	// Enable collision for all boxes and disable for head
	for (auto& FramePackage : FramePackages)
	{
...
void ULagCompensationComponent::ShotgunServerScoreRequest_Implementation(const TArray<ABlasterCharacter*>& HitCharacters, const FVector_NetQuantize TraceStart, const TArray<FVector_NetQuantize>& HitLocations, float HitTime)
{
	...
	for (auto& HitCharater : HitCharacters)
	{
		if (HitCharater == nullptr || Character->GetEquippedWeapon() == nullptr || Character == nullptr)
		{
			continue;
		}

		float TotalDamage = 0.f;
		if (Confirm.HeadShots.Contains(HitCharater))
		{
			float HeadShotDamage = Confirm.HeadShots[HitCharater] * Character->GetEquippedWeapon()->GetHeadShotDamage();
			TotalDamage += HeadShotDamage;
		}
		if (Confirm.BodyShots.Contains(HitCharater))
		{
			float BodyShotDamage = Confirm.BodyShots[HitCharater] * Character->GetEquippedWeapon()->GetDamage();
			TotalDamage += BodyShotDamage;
		}

		UGameplayStatics::ApplyDamage(HitCharater, TotalDamage, Character->Controller, Character->GetEquippedWeapon(), UDamageType::StaticClass());
	}
}
